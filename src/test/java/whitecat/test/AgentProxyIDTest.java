...
public class AgentProxyIDTest {
	...
	List<AgentProxyID> ids = new LinkedList<AgentProxyID>();
	
	// generate the ids
	for( int i = 0; i < numTest; i++ ){
	    AgentProxyID id = AgentProxyID.getNextAgentProxyID();
	    if( ids.contains(id) )
		fail("Found two identical IDs: " + id + " generated at iteration " + i +" and " + ids.indexOf(id) );
	    else
		ids.add(id);
	}
	...
	// create two agent proxy id, check they are not equal and then copy one on the other
	AgentProxyID id1 = AgentProxyID.getNextAgentProxyID();
