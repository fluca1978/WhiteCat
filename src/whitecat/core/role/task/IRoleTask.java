 ...
 * the role aim(s). The idea is that each role has one or more tasks associated to it
 * that can be used to dynamically execute one or more role operations.
 * Tasks can be nested together, so that complex tasks (i.e., complex operations)
 * can be executed at once.
 * A task can be configured (if needed) adding, one at the time, all the parameters
 * (values) that must be used at the execution time.
...
public interface IRoleTask {
    ...
    /**
     * Adds a parameter for the task execution. This method should be used to prepare
     * parameters before the task is effectively executed.
     * Please note that this method must be called with the right sequence in order
     * to respect the sequence of parameters. This means that if you have to add, for instance,
     * two parameters called <i>par1</i> and <i>par2</i> you should use this method as follows:
     * <br>
     * <code>
